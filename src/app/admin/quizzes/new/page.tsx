...
export default function NewQuizPage() {
  ...
  type BankQ = { id:string; kind: SlotKind; prompt:string; options:any; match_id:string; correct?: any }
  ...
  async function buildHistoryPool() {
    try {
      ...
      const { data } = await s
        .from('quiz_questions')
        .select('id,kind,prompt,options,match_id,correct')
        .not('quiz_id', 'is', null)
        .in('kind', ['history_single','history_numeric'])
      const pool = data || []
      setHistoryPool(pool as any)
      if (pool.length) pickRandomHistory(pool as any)
  ...
  const usingHistoryBank = historySel.length > 0
  const historyList = usingHistoryBank ? historySel : slots

  function updateHistoryItem(index:number, data: Partial<Slot>) {
    if (usingHistoryBank) {
      setHistorySel(prev => prev.map((s,i)=> i===index ? ({ ...s, ...data }) : s))
    } else {
      updateSlot(index, data)
    }
  }

  function addHistoryOption(index:number) {
    const current = Array.isArray(historyList[index].options) ? historyList[index].options : []
    updateHistoryItem(index, { options: [...current, ''] })
  }

  function setHistoryOption(index:number, optionIndex:number, value:string) {
    const current = Array.isArray(historyList[index].options) ? [...historyList[index].options] : []
    current[optionIndex] = value
    updateHistoryItem(index, { options: current })
  }

  function removeHistoryOption(index:number, optionIndex:number) {
    const current = Array.isArray(historyList[index].options) ? [...historyList[index].options] : []
    const removed = current.splice(optionIndex, 1)
    const wasCorrect = removed[0] && historyList[index].correct === removed[0]
    updateHistoryItem(index, { options: current, correct: wasCorrect ? null : historyList[index].correct })
  }

  function setHistoryCorrect(index:number, value:any) {
    updateHistoryItem(index, { correct: value })
  }
...
function matchKey(home?: string | null, away?: string | null, kickoff?: string | null) {
...
function toKickoffIso(event: TheSportsDbEvent): string {
  if (event.strTimestamp) {
    const ts = new Date(event.strTimestamp)
